...
func TestVaultTransitKeyProviderUsesDefaultKeyName(t *testing.T) {
	provider, err := NewVaultTransitKeyProvider(VaultTransitKeyProviderConfig{
	...
	if got := provider.encryptPath(); got != "/v1/transit/encrypt/eko-session" {
		t.Fatalf("encryptPath() = %q, want %q", got, "/v1/transit/encrypt/eko-session")
	}
	if got := provider.decryptPath(); got != "/v1/transit/decrypt/eko-session" {
		t.Fatalf("decryptPath() = %q, want %q", got, "/v1/transit/decrypt/eko-session")
	}
}

func TestVaultTransitKeyProviderUnwrapUsesStoredKeyID(t *testing.T) {
	masterKey := make([]byte, aeadKeySize)
	copy(masterKey, []byte("0123456789abcdef0123456789abcdef"))
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "/v1/transit/decrypt/new-session":
			t.Fatalf("unwrap should use stored key id path, got %s", r.URL.Path)
		default:
			t.Fatalf("unexpected path %s", r.URL.Path)
		}
	}))
	...
	writer, err := NewVaultTransitKeyProvider(VaultTransitKeyProviderConfig{
		Address: server.URL,
		Token:   "token",
		Mount:   "transit",
	...
	wrapped, nonce, keyID, err := writer.WrapDataKey(context.Background(), []byte("plaintext-data-key-32-bytes!!!!"))
	if err != nil {
		t.Fatalf("wrap: %v", err)
	}
	if len(nonce) != 0 {
	...
	reader, err := NewVaultTransitKeyProvider(VaultTransitKeyProviderConfig{
		Address: server.URL,
		Token:   "token",
		Mount:   "transit",
		KeyName: "new-session",
	...
	unwrapped, err := reader.UnwrapDataKey(context.Background(), keyID, wrapped, nil)
	if err != nil {
		t.Fatalf("unwrap: %v", err)
	}
	if string(unwrapped) != "plaintext-data-key-32-bytes!!!!" {
